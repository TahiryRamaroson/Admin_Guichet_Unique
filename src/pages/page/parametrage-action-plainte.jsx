...
import {
  ...
  export function ParametrageActionPlainte() {
    ...
    const handleOpenSuccess = () => setOpenSuccess(!open);
      ...
      const getActionPlainte = async (pageNumber) => {
  
        const apiActionPlainte = `${api_url}/api/Actions/page/${pageNumber}`; 
        ...
        try {
          ...
          setDataActionPlainte(data.action);
      ...
      const deleteActionPlainte = async (id) => {
  
        const apiActionPlainte = `${api_url}/api/Actions/${id}`; 
      ...
      const postActionPlainte = async (e) => {
        e.preventDefault();
    
        const apiAjout = `${api_url}/api/Actions`;
      ...
      const putActionPlainte = async (id) => {
      ...
      const getFilteredActionPlainte = async (pageNumber) => {
    
        const apiFiltre = `${api_url}/api/Actions/filtre/page/${pageNumber}`;
      ...
      const downloadCSV = async () => {
        try {
          const response = await fetch(`${api_url}/api/Actions/export/csv`, {
            method: 'GET',
            headers: {
              'Authorization': 'Bearer ' + sessionStorage.getItem('authToken'),
            },
          ...
          a.href = url;
          a.download = `Actions_${new Date().toISOString()}.csv`;
      ...
      const downloadExcel = async () => {
        try {
          const response = await fetch(`${api_url}/api/Actions/export/excel`, {
            method: 'GET',
            headers: {
              'Authorization': 'Bearer ' + sessionStorage.getItem('authToken'),
            },
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `Actions_${new Date().toISOString()}.xlsx`;
          document.body.appendChild(a);
          a.click();
          a.remove();
        } catch (error) {
      ...
      const importCSV = async (event) => {
        ...
        try {
          const response = await axios.post(`${api_url}/api/Actions/import/csv`, formData, {
            headers: {
      ...
      const importExcel = async (event) => {
        ...
        try {
          const response = await axios.post(`${api_url}/api/Actions/import/excel`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
              'Authorization': 'Bearer ' + sessionStorage.getItem('authToken'),
            },
    ...
    useEffect(() => {
      if (isFiltered) {
        getFilteredActionPlainte(pageNumber);
        } else {
        getActionPlainte(pageNumber);
        }
    ...
    <Card className="h-full w-full">
        ...
        <div className="mt-5 mb-5 flex items-center justify-center">
          ...
          <Dialog open={openAjout} handler={handleOpenAjout} size="sm">
            <DialogBody>
              <form onSubmit={postActionPlainte} className="mt-2 mb-2 w-full">
      ...
      <CardBody className="overflow-scroll px-0">
        <table className="mt-6 w-full min-w-max table-auto">
          ...
          <tbody>
            {dataActionPlainte && dataActionPlainte.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="p-4 border-b border-blue-gray-50">
                      ...
                      <Tooltip content="Supprimer">
                          <Button onClick={() => deleteActionPlainte(item.id)} className="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center ml-5">
              ...
              <Dialog open={openModif} handler={handleOpenModif} size="sm">
                <DialogBody>
                  <form onSubmit={() => putActionPlainte(idModif)} className="mt-2 mb-2 w-full">
                    <Card className="w-full">
                    <Typography variant="h3" color="black" className="mt-6 flex justify-center gap-1 font-normal">
                      Modification
                    </Typography>
  ...
  export default ParametrageActionPlainte;
