...
import {
  ...
  export function ParametrageAntecedentMedical() {
    ...
    const handleOpenSuccess = () => setOpenSuccess(!open);
      ...
      const getAntecedentMedicaux = async (pageNumber) => {
  
        const apiAntecedentMedicaux = `${api_url}/api/AntecedentMedicaux/page/${pageNumber}`; 
        ...
        try {
          ...
          setDataAntecedentMedicaux(data.antecedentMedicaux);
      ...
      const deleteAntecedentMedicaux = async (id) => {
  
        const apiAntecedentMedicaux = `${api_url}/api/AntecedentMedicaux/${id}`; 
        ...
        try {
          ...
          getAntecedentMedicaux(pageNumber);
          console.log("dataAntecedentMedicaux après la mise à jour d'état :", data);
        } catch (error) {
          console.error("Error: " + error.message);
        }
      ...
      const postAntecedentMedicaux = async (e) => {
        e.preventDefault();
    
        const apiAjout = `${api_url}/api/AntecedentMedicaux`;
      ...
      const putAntecedentMedicaux = async (id) => {
      ...
      const getFilteredAntecedentMedicaux = async (pageNumber) => {
    
        const apiFiltre = `${api_url}/api/AntecedentMedicaux/filtre/page/${pageNumber}`;
      ...
      const downloadCSV = async () => {
        try {
          const response = await fetch(`${api_url}/api/AntecedentMedicaux/export/csv`, {
            method: 'GET',
            headers: {
              'Authorization': 'Bearer ' + sessionStorage.getItem('authToken'),
            },
          ...
          a.href = url;
          a.download = `AntecedentMedicaux_${new Date().toISOString()}.csv`;
      ...
      const downloadExcel = async () => {
        try {
          const response = await fetch(`${api_url}/api/AntecedentMedicaux/export/excel`, {
            method: 'GET',
            headers: {
              'Authorization': 'Bearer ' + sessionStorage.getItem('authToken'),
            },
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `AntecedentMedicaux_${new Date().toISOString()}.xlsx`;
          document.body.appendChild(a);
          a.click();
          a.remove();
        } catch (error) {
      ...
      const importCSV = async (event) => {
        ...
        try {
          const response = await axios.post(`${api_url}/api/AntecedentMedicaux/import/csv`, formData, {
            headers: {
      ...
      const importExcel = async (event) => {
        ...
        try {
          const response = await axios.post(`${api_url}/api/AntecedentMedicaux/import/excel`, formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
              'Authorization': 'Bearer ' + sessionStorage.getItem('authToken'),
            },
    ...
    useEffect(() => {
      if (isFiltered) {
        getFilteredAntecedentMedicaux(pageNumber);
        } else {
        getAntecedentMedicaux(pageNumber);
        }
    ...
    <Card className="h-full w-full">
        ...
        <div className="mt-5 mb-5 flex items-center justify-center">
          ...
          <Dialog open={openAjout} handler={handleOpenAjout} size="sm">
            <DialogBody>
              <form onSubmit={postAntecedentMedicaux} className="mt-2 mb-2 w-full">
      ...
      <CardBody className="overflow-scroll px-0">
        <table className="mt-6 w-full min-w-max table-auto">
          ...
          <tbody>
            {dataAntecedentMedicaux && dataAntecedentMedicaux.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="p-4 border-b border-blue-gray-50">
                      ...
                      <Tooltip content="Supprimer">
                          <Button onClick={() => deleteAntecedentMedicaux(item.id)} className="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center ml-5">
              ...
              <Dialog open={openModif} handler={handleOpenModif} size="sm">
                <DialogBody>
                  <form onSubmit={() => putAntecedentMedicaux(idModif)} className="mt-2 mb-2 w-full">
                    <Card className="w-full">
                    <Typography variant="h3" color="black" className="mt-6 flex justify-center gap-1 font-normal">
                      Modification
                    </Typography>
  ...
  export default ParametrageAntecedentMedical;
